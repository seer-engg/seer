...
import { getToolAccounts, type ToolAccountInfo, type ToolAccountsResponse } from '@/lib/api-client';
...
function useToolAccounts(toolName: string, selectedConnectionId: number | null | undefined, onSelect: (id: number | null) => void) {
  const [loading, setLoading] = useState(false);
  const [accountsData, setAccountsData] = useState<ToolAccountsResponse | null>(null);
  const [fetchError, setFetchError] = useState<string | null>(null);

  useEffect(() => {
    if (!toolName) { setAccountsData(null); return; }
    let cancelled = false;
    setLoading(true);
    setFetchError(null);
    getToolAccounts(toolName)
  ...
  useEffect(() => {
    if (accountsData && accountsData.accounts.length >= 1 && selectedConnectionId == null) {
...
/** Account selector dropdown for multi-account OAuth support. */
export function AccountSelector({ toolName, selectedConnectionId, onSelect, disabled = false, error, onConnectAccount }: AccountSelectorProps) {
  const [open, setOpen] = useState(false);
  const { loading, accountsData, fetchError } = useToolAccounts(toolName, selectedConnectionId, onSelect);
