            ...
            <Tooltip key={integration}>
              <TooltipTrigger asChild>
                <span className="capitalize">
                  <IntegrationIcon integrationType={integration} />
                </span>
              </TooltipTrigger>
              <TooltipContent>
                {useIntegrationMetadataStore.getState().getDisplayName(integration)}
              </TooltipContent>
            </Tooltip>
          ))}
        </div>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                size="icon"
                className="h-7 w-7"
                onClick={() => onExport(workflow.workflow_id)}
              >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                size="icon"
                className="h-7 w-7"
                onClick={() => onRenameStart(workflow)}
              >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                onClick={() => onDelete({ id: workflow.workflow_id, name: workflow.name })}
              >
...
function useWorkflowActions() {
  ...
  const handleCreateNew = useCallback(
    async (isCreating: boolean, setIsCreating: (v: boolean) => void) => {
  ...
  const handleRenameSave = useCallback(
    async (workflowId: string, renameValue: string, setRenamingId: (v: string | null) => void) => {
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          ...
          <Button
            size="sm"
            variant="brand"
            onClick={() => actions.handleCreateNew(isCreating, setIsCreating)}
            disabled={isCreating}
          >
            {isCreating ? (
