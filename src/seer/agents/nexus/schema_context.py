...
def generate_trigger_reference() -> str:
    ...
    lines.append("- Trigger data accessed via `${<trigger_id>.event}` — use the trigger's `id`, NOT the word 'trigger'")
    lines.append("  - Example: if trigger id is `gmail_new`, use `${gmail_new.event.subject}`, NOT `${trigger.data.subject}`")
