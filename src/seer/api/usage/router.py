...
class FailureRateMetric(BaseModel):
    total_runs: int
    failed_runs: int
    failure_rate: float


class StepFailureItem(BaseModel):
    node_id: str
    node_type: str
    error_count: int


class DryRunAdoption(BaseModel):
    total_workflows: int
    previewed_workflows: int
    adoption_rate: float
    status: str


class KPISummaryResponse(BaseModel):
    weekly_active_workflows: int
    time_to_first_workflow_seconds: Optional[float] = None
    workflow_failure_rate: FailureRateMetric
    dry_run_adoption: DryRunAdoption


class StepFailureResponse(BaseModel):
    period_days: int
    nodes: list[StepFailureItem]


@router.get("/kpis", response_model=KPISummaryResponse)
async def get_kpi_summary(request: Request) -> KPISummaryResponse:
    """Top 4 strategic KPIs: WAW, TTFW, failure rate, dry-run adoption."""
    from seer.observability.kpi import get_kpi_summary as _get_kpi_summary  # pylint: disable=import-outside-toplevel  # Reason: avoid circular import

    user = _require_user(request)
    organization: Optional[Organization] = getattr(request.state, "organization", None)

    data = await _get_kpi_summary(user=user, organization=organization)
    return KPISummaryResponse(
        weekly_active_workflows=data["weekly_active_workflows"],
        time_to_first_workflow_seconds=data.get("time_to_first_workflow_seconds"),
        workflow_failure_rate=FailureRateMetric(**data["workflow_failure_rate"]),
        dry_run_adoption=DryRunAdoption(**data["dry_run_adoption"]),
    )


@router.get("/kpis/step-failures", response_model=StepFailureResponse)
async def get_step_failures(
    request: Request,
    days: int = Query(default=7, ge=1, le=90, description="Lookback window in days"),
) -> StepFailureResponse:
    """Per-node failure breakdown from workflow run traces."""
    from seer.observability.kpi import get_step_failure_breakdown  # pylint: disable=import-outside-toplevel  # Reason: avoid circular import

    user = _require_user(request)
    organization: Optional[Organization] = getattr(request.state, "organization", None)

    nodes = await get_step_failure_breakdown(user=user, organization=organization, days=days)
    return StepFailureResponse(
        period_days=days,
        nodes=[StepFailureItem(**n) for n in nodes],
    )
