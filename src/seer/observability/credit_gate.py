...
async def _check_overage_allowance(
    ...
    For team organizations, checks org-level overage settings.
    ...
    """
    # Get effective overage settings (org settings for team orgs, user settings otherwise)
...
async def _check_org_single_limit(
    organization: Organization,
    limit: float,
    period: LimitPeriod,
    tier: SubscriptionTier,
    get_usage_fn: Callable[[Organization], Awaitable[Decimal]],
) -> None:
    """
    Check a single credit limit for an organization and raise/warn as appropriate.

    Args:
        organization: The organization to check
        limit: The credit limit for this period
        period: The time period (5-hour, weekly, or monthly)
        tier: The organization's subscription tier
        get_usage_fn: Async function to retrieve current usage for this period

    Raises:
        CreditLimitExceeded: If organization is at or over the hard threshold (120%)
    """
    credits_used = await get_usage_fn(organization)
    limit_decimal = Decimal(str(limit))
    ...
    if credits_used >= soft_threshold:
        percentage = (credits_used / limit_decimal) * Decimal("100")
        logger.warning(
            "Organization %s approaching %s LLM credit limit: $%.2f / $%.2f (%.1f%%)",
            organization.id,
            period.value,
            credits_used,
            limit,
...
async def _check_monthly_limit_with_overage(
    ...
    """
    Check monthly credit limit with overage support for paid tier users/orgs.

    For team organizations, checks org-level usage and overage settings.
    ...
    Args:
        ...
        tier: The user's/org's subscription tier
        organization: Optional organization context (for team billing)
    ...
    # Get usage: org-level for team orgs, user-level otherwise
    if use_org_limits:
        credits_used = await get_org_monthly_llm_credits_used(organization)
    else:
    ...
    if credits_used < hard_threshold:
        ...
        if credits_used >= soft_threshold:
            ...
            if use_org_limits and organization:
                logger.warning(
                    "Organization %s approaching monthly LLM credit limit: $%.2f / $%.2f (%.1f%%)",
                    organization.id,
                    credits_used,
                    limit,
                    percentage,
    ...
    overage_result = await _check_overage_allowance(user, credits_used, limit, organization)

    if overage_result.allowed:
        # Overage enabled and has remaining capacity
        if use_org_limits and organization:
            logger.info(
                "Organization %s using overage credits: $%.2f used, $%.2f remaining in cap",
                organization.id,
                credits_used,
...
async def check_credit_limit(user: User, organization: Optional[Organization] = None) -> None:
    """
    Check if user/organization has sufficient LLM credits before execution.
    ...
    For team organizations, checks org-level limits and usage.
    For monthly limits, paid tier users/orgs with overage enabled can exceed
    ...
    Raises:
        CreditLimitExceeded: If user/org is at or over 120% of any limit
                            (or overage cap reached for monthly)

    Logs warning if user/org is at or over 80% of any limit.
    """
    # Use effective limits (org limits for team orgs, user limits otherwise)
    ...
    # Determine if we should use org-level usage checks
    use_org_limits = organization and organization.type == OrganizationType.TEAM

    # Check 5-hour limit (burst protection) - no overage support
    if not limits.has_unlimited_5h_credits:
        if use_org_limits and organization:  # organization is guaranteed non-None here
            await _check_org_single_limit(
                organization=organization,
                limit=limits.llm_credits_5h,
                period=LimitPeriod.FIVE_HOUR,
                tier=tier,
                get_usage_fn=get_org_5h_llm_credits_used,
            )
        else:
    ...
    if not limits.has_unlimited_weekly_credits:
        if use_org_limits and organization:  # organization is guaranteed non-None here
            await _check_org_single_limit(
                ...
                tier=tier,
                get_usage_fn=get_org_weekly_llm_credits_used,
            )
        else:
            await _check_single_limit(
                user=user,
