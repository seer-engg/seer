...
def _extract_metadata_from_checkpoint_dict(checkpoint: Dict[str, Any]) -> Dict[str, Any]:
    """Extract metadata from checkpoint metadata field."""
    checkpoint_metadata = checkpoint.get("metadata", {})
    if isinstance(checkpoint_metadata, dict):
        return {
            "workflow_id": checkpoint_metadata.get("workflow_id"),
            "title": checkpoint_metadata.get("title")
        }
    return {}


def _extract_metadata_from_channel_values(channel_values: Dict[str, Any]) -> Dict[str, Any]:
    """Extract metadata from channel_values metadata field."""
    if "metadata" in channel_values and isinstance(channel_values["metadata"], dict):
        return {
            "workflow_id": channel_values["metadata"].get("workflow_id"),
            "title": channel_values["metadata"].get("title")
        }
    return {}


def _extract_title_from_first_message(channel_values: Dict[str, Any]) -> Optional[str]:
    """Extract title from first message in channel_values."""
    ...
    return content[:100].strip() if content else None
...
def _extract_metadata_from_checkpoint(
    ...
    # Start with checkpoint metadata
    metadata = _extract_metadata_from_checkpoint_dict(checkpoint)

    # Fill in missing fields from channel_values metadata
    channel_metadata = _extract_metadata_from_channel_values(channel_values)
    if not metadata.get("workflow_id"):
        metadata["workflow_id"] = channel_metadata.get("workflow_id")
    if not metadata.get("title"):
        metadata["title"] = channel_metadata.get("title")

    # Fallback: extract workflow_id from thread_id if still missing
    ...
    # Fallback: extract title from first message if still missing
    if not metadata.get("title"):
        title = _extract_title_from_first_message(channel_values)
        if title:
...
def _parse_checkpoint_timestamp(ts_str: str) -> datetime:
    """Parse timestamp string from checkpoint, with fallback to current time."""
    try:
...
def _should_skip_thread(thread_id: Optional[str]) -> bool:
    """Check if thread should be skipped (missing or workflow execution)."""
    if not thread_id:
        return True
    # Skip workflow executions (run_* pattern)
    return thread_id.startswith("run_")


async def _build_trace_summary(thread_id: str, data: Dict[str, Any]) -> AgentTraceSummary:
    """Build AgentTraceSummary from thread data."""
...
async def _collect_thread_checkpoints(checkpointer) -> Dict[str, Dict[str, Any]]:
    """Collect all checkpoints grouped by thread_id."""
    ...
    async for checkpoint_tuple in checkpointer.alist({}):
        ...
        timestamp = _parse_checkpoint_timestamp(ts_str)
...
async def list_agent_traces(
    ...
    try:
        # Collect all checkpoints grouped by thread
        thread_data = await _collect_thread_checkpoints(checkpointer)
...
async def _find_earliest_checkpoint_timestamp(checkpointer, config: Dict[str, Any], latest_ts: datetime) -> datetime:
    """Find earliest timestamp across all checkpoints for a thread."""
    earliest_ts = latest_ts
    async for checkpoint_tuple in checkpointer.alist(config):
        cp_ts_str = checkpoint_tuple.checkpoint.get("ts", "")
        cp_ts = _parse_checkpoint_timestamp(cp_ts_str)
        if cp_ts < earliest_ts:
            earliest_ts = cp_ts
    return earliest_ts


def _convert_messages_to_agent_messages(messages_raw: Any, ts_str: str) -> List[AgentMessage]:
...
async def get_agent_trace(
    ...
    try:
        ...
        # Find earliest timestamp across all checkpoints
        earliest_ts = await _find_earliest_checkpoint_timestamp(checkpointer, config, timestamp)

        # Extract messages
        messages_raw = channel_values.get("messages", [])
        messages = _convert_messages_to_agent_messages(messages_raw, ts_str)
