...
async def _stream_events_mode(graph, input_data: Dict[str, Any], config: Dict[str, Any]) -> AsyncGenerator[str, None]:
    """Stream full event stream."""
    async for event in graph.astream_events(input_data, config=config, version="v2"):
        yield _format_sse_event(event["event"], event)


async def _stream_messages_mode(graph, input_data: Dict[str, Any], config: Dict[str, Any]) -> AsyncGenerator[str, None]:
    """Stream LLM tokens and chain events."""
    async for event in graph.astream_events(input_data, config=config, version="v2"):
...
async def _stream_updates_mode(graph, input_data: Dict[str, Any], config: Dict[str, Any]) -> AsyncGenerator[str, None]:
    ...
    async for chunk in graph.astream(input_data, config=config, stream_mode="updates"):
        for node_name, update in chunk.items():
            ...
            yield _format_sse_event("update", {"node": node_name, "data": serialized})


async def _stream_values_mode(graph, input_data: Dict[str, Any], config: Dict[str, Any]) -> AsyncGenerator[str, None]:
...
async def _stream_graph_events(
    ...
    try:
        if stream_mode == "events":
            async for event in _stream_events_mode(graph, input_data, config):
                yield event
        elif stream_mode == "messages":
            async for event in _stream_messages_mode(graph, input_data, config):
                yield event
        elif stream_mode == "updates":
            async for event in _stream_updates_mode(graph, input_data, config):
                yield event
        else:  # stream_mode == "values"
            async for event in _stream_values_mode(graph, input_data, config):
                yield event
