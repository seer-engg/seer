...
# Node type descriptors for frontend builder UI
...
def _collect_warnings_from_nodes(nodes: Iterable[Node]) -> List[api_models.WorkflowWarning]:
    """
    Recursively collect warnings from workflow nodes.

    Checks for common issues like missing 'out' fields on nodes.
    """
...
def _graph_preview(spec: WorkflowSpec) -> Dict[str, Any]:
    """Generate simple graph preview from workflow spec."""
...
async def list_node_types() -> api_models.NodeTypeResponse:
    """List available node types for workflow builder."""
...
async def list_tools(include_schemas: bool = False) -> api_models.ToolRegistryResponse:
    ...
    Args:
        include_schemas: Include input/output JSON schemas

    Returns:
        Tool registry response with tool descriptors
    """
...
async def list_triggers() -> api_models.TriggerCatalogResponse:
    """List available trigger definitions from registry."""
    triggers = [
...
async def list_models() -> api_models.ModelRegistryResponse:
    ...
    Includes default models plus any configured default model.
    """
...
async def resolve_schema(schema_id: str) -> api_models.SchemaResponse:
    """
    Resolve a schema by ID from the compiler's schema registry.

    Args:
        schema_id: Schema identifier

    Returns:
        Schema response with JSON schema
...
async def compile_spec(user: User, payload: api_models.CompileRequest) -> api_models.CompileResponse:
    """
    Compile workflow specification.

    Args:
        user: User context
        payload: Compilation request with spec and options

    Returns:
        Compilation response with warnings and optional artifacts
