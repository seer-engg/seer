...
def test_build_type_environment_minimal():
    ...
    # 'vars' and 'variables' (alias) are always registered for global variable support
    assert len(env.as_dict()) == 2
    assert "vars" in env.as_dict()
    assert "variables" in env.as_dict()
...
def test_build_type_environment_with_agent_node():
    ...
    spec = WorkflowSpec(
        ...
        nodes=[
            AgentNode(
                id="llm1",
                inputs={"model": "qwen/qwen3-235b-a22b-2507", "prompt": "Generate a response"},
...
def test_for_each_loop_variable_infers_type_from_agent_node_output():
    ...
    spec = WorkflowSpec(
        ...
        nodes=[
            AgentNode(
                id="prepare_data",
                inputs={"model": "qwen/qwen3-235b-a22b-2507", "prompt": "Generate organizations"},
