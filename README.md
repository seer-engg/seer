...
| **testing (integrations)** | `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET` | Required for OAuth integrations (Gmail, Google Drive, GitHub). Frontend controls OAuth scopes (read-only is core differentiation). |
...
| **optional tracing** | `MLFLOW_TRACKING_URI`, `MLFLOW_EXPERIMENT_NAME` | Enables MLflow traces (recommended for debugging). |
